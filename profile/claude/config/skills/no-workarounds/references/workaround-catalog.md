...
The core message is: write code that actively verifies its own assumptions, rather than simply assuming they are true.

Whenever you think "but that could never happen", add code to verify exactly that.
Assertions are checks for conditions that should never be false — they document and enforce your assumptions.

## When Not to Use Assertions

Do not use assertions in place of real error handling. They exist to catch bugs in your own code, not to validate user input:

```tsx
// WORKAROUND: Lying to the compiler : Bad Pattern
function renderSubscriptionView() {
    if (isLoading) return <SkeletonView />
    if (!subscription) return <ErrorAreaController />
    assertSubscription(subscription)
    switch (subscription.type) {
      case "postpaid":
        return <PostpaidUseCase subscription={subscription as Postpaid} />
      case "control":
        return <ControlUseCase subscription={subscription as Control} />
      case "fixed-siebel":
        return <SiebelUseCase subscription={subscription as Siebel} />
      default:
        return <ErrorAreaController />
    }
 }

// PROPER FIX: Make types truthful: assertive programing
 function renderSubscriptionView() {
    if (isLoading) return <SkeletonView />
    if (!subscription) return <ErrorAreaController />
    assertSubscription(subscription)
    switch (subscription.type) {
      case "postpaid":
        assertPostpaidSubscription(subscription) // assert function
        return <PostpaidUseCase subscription={subscription} />
      case "control":
        assertControlSubscription(subscription) // assert function
        return <ControlUseCase subscription={subscription} />
      case "fixed-siebel":
        assertSiebelSubscription(subscription) // assert function
        return <SiebelUseCase subscription={subscription} />
      default:
        return <ErrorAreaController />
    }
 }

 // assert functions says to compiler that`s a valid assertion type
 // Wrong type never will pass in this flow
 function assertPostpaidSubscription(
  anObject: any,
  ): asserts anObject is Postpaid {
    if (anObject?.type !== SUBSCRIPTION_TYPES.postpaid) {
      throw new Error("Invalid Postpaid Subscription")
    }
  }

  function assertControlSubscription(anObject: any): asserts anObject is Control {
    if (anObject?.type !== SUBSCRIPTION_TYPES.control) {
      throw new Error("Invalid Control Subscription")
    }
  }

  function assertSiebelSubscription(anObject: any): asserts anObject is Siebel {
    if (anObject?.type !== SUBSCRIPTION_TYPES["fixed-siebel"]) {
      throw new Error("Invalid Siebel Subscription")
    }
  }
```
